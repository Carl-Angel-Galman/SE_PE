...
Der System State Manager besitzt einen Verweis auf den aktuellen Zustand in Form eines Enums \emph{current\_state}. Die Klasse \emph{IState} 
ist eine Abstrakte Klasse und definiert die abstrakten Methoden \emph{onEntry}, \emph{onCyclic} und \emph{onExit}, 
...
Der TTC-Observer wird auf dem Jetson AGX implementiert, da für die TTC Berechnung die hochauflösenden Distanz benötigt werden, welche direkt von den 
\emph{Intel Realsense} auf dem Jetson AGX verarbeitet werden. Aus diesem Knoten wird auch 
...
Der Arduino \emph{Slave} ist auch mit dem Empfänger für die 
\emph{Funkfernsteuerng(Empfänger)} verbunden, welcher die Stop- und Fahrfreigabesignale von der \emph{Mission Control} empfängt,
daher wird das \emph{Emergency Stop} Subsystem auf diesem Knoten implementiert, da das \emph{Emergency-Modul}
eine direkte Schnittstelle zu \emph{Mission-Control} benötigt.

Der Arduino \emph{Slave} dient also als zentrale Steuereinheit für die Not-Aus-Funktion, da er direkten Zugriff auf die relevanten Hardwarekomponenten hat und die Signale von der Fernsteuerung empfängt.
Damit erschließt sich auch als sinnvoll, das \emph{logging} in dieem Knoten zu implementieren, da hier alle relevanten Informationen für die Not-Aus-Funktion gesammelt werden können.
